...
const App = () => {
  const navigate = useNavigate()  // ✅ تعريف
...
useEffect (() =>{
  ...
  const handelClick = (movieId) => {  // ✅ استقبل الـ ID
    navigate(`/movie/${movieId}`)  // ✅ Route صحيحة
  }
  ...
  return (
    ...
    <main>
          ...
          <section className='trending'>
           ...
           <ul>
            {trendingMovies.map((movie, index)=>(
              <li key={movie.$id} onClick={() => handelClick(movie.id)}>  {/* ✅ pass the id */}
                <p>{index + 1}</p>
      ...
      />
      <Route path='/movie/:id' element={<MovieDetails />} />  {/* ✅ Route صحيحة */}
    </Routes>
  )
}
